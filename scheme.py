...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(proced, MacroProcedure):
            return scheme_eval(proced.apply_macro(rest, env), env)
        fn = lambda x: scheme_eval(x, env)
        rest = rest.map(fn)
        return scheme_apply(proced, rest,env)
        # END PROBLEM 5
...
def eval_all(expressions, env, t= False):
    ...
    if not expressions:
        ...
        return scheme_eval(expressions.first, env, t)
    else:
        scheme_eval(expressions.first, env)
        return eval_all(expressions.second, env, t)
    # END PROBLEM 8
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(first, do_lambda_form(Pair(second, body), env))
...
def do_lambda_form(expressions, env):
    ...
    return LambdaProcedure(formals, sec, env)
    # END PROBLEM 9
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        check_formals(formals)
        res = Pair(scheme_eval(bindings.first.second.first, env),res)
        bindings = bindings.second
    return env.make_child_frame(formals, res)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    check_formals(res)
    check_form(res, 0)
    fn, form, body = res.first, res.second, expressions.second
    env.define(fn, MacroProcedure(form, body, env))
    return fn
    # END Problem 21
